# keras32_MCP_load_02_diabetes.py
# 구조 : keras30_ModelCheckPoint2_load.py 기준
# 짝 파일 : keras31_MCP_save_02_diabetes.py -> 반드시 그 파일을 먼저 실행해서 keras31_mcp2.keras 를 만들어야 한다.
...
#1. 데이터
# #1 은 keras31 과 완전히 같아야 한다.
# 모델은 '그때 변환된 x' 에 맞춰 학습됐으므로 split / random_state / scaler 중 하나만 달라도 결과가 달라진다.
...
# 훈련을 안 하는데도 이 split 은 지우면 안 된다.
# 아래 scaler 가 '이 split 뒤의 x_train' 으로 fit 해야 keras31 과 같은 기준으로 x_test 를 변환한다.
...
# .keras 파일에는 구조 + 가중치 + compile 정보가 같이 저장돼 있어서 Sequential / compile 이 필요 없다.
# fit 을 하면 불러온 가중치에서 또 학습해버려서 keras31 과 값이 달라지므로 fit 도 하지 않는다.
model = load_model(path + 'keras31_mcp2.keras')
...
# 불러온 모델 = keras31 에서 val_loss 가 가장 좋았던 epoch 의 모델이고, fit 을 안 했으니 가중치도 keras31_mcp2.keras 와 같다.
# 파일명 만들기는 keras30_ModelCheckPoint3 방식 : 폴더 + 접두어 + 날짜 + 결과값
date = datetime.datetime.now().strftime('%m%d_%H%M')   # 실행한 날짜_시간 (예: 0914_1850)
filepath = ''.join([path, 'k32_02_', date, '_', f'{loss:.4f}', '.keras'])   # x_test loss 를 소수 4자리로
# 예) ./_save/keras31/k32_02_0914_1850_0.1234.keras -> 이름만 보고 어떤 결과를 낸 모델인지 알 수 있다
model.save(filepath)
