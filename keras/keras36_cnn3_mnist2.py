...
model = Sequential()
model.add(Conv2D(32, (3,3), activation='relu', input_shape=(28, 28, 1)))    # 출력 : (26, 26, 32)
model.add(Conv2D(32, kernel_size=(3,3), activation='relu'))                 # 출력 : (24, 24, 32)
model.add(Conv2D(32, kernel_size=(5,5), activation='relu'))                 # 출력 : (20, 20, 32)  (5,5) 는 4씩 줄어든다
model.add(Dropout(0.25))
model.add(Conv2D(64, kernel_size=(3,3), activation='relu'))                 # 출력 : (18, 18, 64)
model.add(Conv2D(64, kernel_size=(3,3), activation='relu'))                 # 출력 : (16, 16, 64)
model.add(Conv2D(64, kernel_size=(5,5), activation='relu'))                 # 출력 : (12, 12, 64)
model.add(Dropout(0.25))
model.add(Conv2D(128, kernel_size=(3,3), activation='relu'))                # 출력 : (10, 10, 128)
model.add(Dropout(0.25))
model.add(Flatten())                                                # (10,10,128) -> 12800. 4차원을 2차원으로 펴준다 (값과 순서가 그대로라 reshape 와 같다)

model.add(Dense(units=256, activation='relu'))                      # units = Dense 의 첫 번째 인자 이름 (Dense(256) 과 같다)
model.add(Dropout(0.4))                                             # Flatten 직후가 파라미터가 가장 많은 구간이라 Dropout 을 크게 준다
