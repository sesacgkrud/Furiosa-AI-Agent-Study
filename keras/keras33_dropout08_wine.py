# keras33_dropout08_wine.py
# 구조 : keras33_dropout01_california.py 기준 / 데이터, 모델 : keras31_MCP_save_08_wine.py + Dropout 적용
...
model.add(Dropout(0.4))     # 가장 큰 층(200) 출력의 40% 를 훈련 때마다 랜덤으로 끈다
model.add(Dense(150, activation='relu'))
model.add(Dense(100, activation='relu'))
model.add(Dropout(0.2))     # 100 층 출력의 20% 를 끈다
model.add(Dense(50, activation='relu'))
model.add(Dense(10, activation='relu'))
model.add(Dense(3, activation='softmax'))   # 클래스 3개 -> 3칸 확률 (합 = 1)
...
mcp = ModelCheckpoint(
    ...
    filepath=path + 'keras33_mcp8.keras',   # 파일 번호 규칙 : 08 -> mcp8
    verbose=1,
)
...
# ===== 이전 기록 =====
# Dropout 적용 전 (keras31_MCP_save_08)     loss : 0.08734525740146637 / accuracy_score : 0.9814814814814815
# Dropout 적용 후 (이전 실행, MCP 없음)     loss : 0.08068563044071198 / accuracy_score : 0.9629629629629629

# ===== 실행 결과 (2026-09-14, Dropout + MCP + r2/mse/rmse 추가 후) =====
# loss : 0.10924266278743744
# acc : 0.963
# r2 : 0.9139685899459123
# mse : 0.018718052942593193
# RMSE : 0.1368139354839016
# accuracy_score : 0.9629629629629629
# (Epoch 150: early stopping / Restoring model weights from the end of the best epoch: 100.)
