# keras33_dropout10_digits.py
# 구조 : keras33_dropout01_california.py 기준 / 데이터, 모델 : keras31_MCP_save_10_digits.py + Dropout 적용
...
model.add(Dropout(0.2))     # 층이 작아질수록 끄는 비율도 20% 로 줄인다
model.add(Dense(20, activation='relu'))
model.add(Dense(15, activation='relu'))
model.add(Dropout(0.2))
model.add(Dense(10, activation='softmax'))  # 숫자 0 ~ 9 -> 10칸 확률 (합 = 1)
...
mcp = ModelCheckpoint(
    ...
    filepath=path + 'keras33_mcp10.keras',  # 파일 번호 규칙 : 10 -> mcp10
    verbose=1,
)
...
# ===== 이전 기록 =====
# Dropout 적용 전 (keras31_MCP_save_10)     loss : 0.2588503658771515 / accuracy_score : 0.9513212795549374
# Dropout 적용 후 (이전 실행, MCP 없음)     loss : 0.19250112771987915 / accuracy_score : 0.9652294853963839

# ===== 실행 결과 (2026-09-14, Dropout + MCP + r2/mse/rmse 추가 후) =====
# loss : 0.17552341520786285
# acc : 0.969
# r2 : 0.9398945811522225
# mse : 0.005389126007911048
# RMSE : 0.07341066685374169
# accuracy_score : 0.9694019471488178
# (Epoch 188: early stopping / Restoring model weights from the end of the best epoch: 88.)
