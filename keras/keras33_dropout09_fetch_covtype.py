# keras33_dropout09_fetch_covtype.py
# 구조 : keras33_dropout01_california.py 기준 / 데이터, 모델 : keras31_MCP_save_09_fetch_covtype.py + Dropout 적용
# Dropout : 훈련할 때마다 층 출력의 일부 노드를 랜덤으로 꺼서(0 으로 만들어서) 특정 노드에만 의존하지 않게 한다 -> 과적합 방지
#           evaluate / predict 때는 자동으로 꺼지고 모든 노드를 다 사용한다
# [참고] 데이터 58만 개라 훈련이 오래 걸린다 (Dropout 없는 keras31_09 기준 약 15분)
...
model.add(Dense(100, activation='relu'))
model.add(Dense(100, activation='relu'))
model.add(Dropout(0.2))     # 출력층 가까이는 20% 만 끈다
...
mcp = ModelCheckpoint(
    ...
    filepath=path + 'keras33_mcp9.keras',   # 파일 번호 규칙 : 09 -> mcp9
    verbose=1,
)
...
# ===== 이전 기록 =====
# Dropout 적용 전 (keras31_MCP_save_09)     loss : 0.1659817397594452 / accuracy_score : 0.9426978153111805
# Dropout 적용 후 (이전 실행, MCP 없음)     loss : 0.2004879117012024 / accuracy_score : 0.920346062052506

# ===== 실행 결과 (2026-09-14, Dropout + MCP + r2/mse/rmse 추가 후) =====
# loss : 0.19378028810024261
# acc : 0.924
# r2 : 0.6942853879199242
# mse : 0.013928227671960279
# RMSE : 0.11801791250467142
# accuracy_score : 0.9236391591701855
# (Epoch 124: early stopping / Restoring model weights from the end of the best epoch: 104.)
