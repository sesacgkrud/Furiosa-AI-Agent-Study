# keras32_MCP_load_08_wine.py
# 구조 : keras30_ModelCheckPoint2_load.py 기준
# 짝 파일 : keras31_MCP_save_08_wine.py -> 반드시 그 파일을 먼저 실행해서 keras31_mcp8.keras 를 만들어야 한다.
...
model = load_model(path + 'keras31_mcp8.keras')
...
# 불러온 모델 = keras31 에서 val_loss 가 가장 좋았던 epoch 의 모델이고, fit 을 안 했으니 가중치도 keras31_mcp8.keras 와 같다.
# 파일명 만들기는 keras30_ModelCheckPoint3 방식 : 폴더 + 접두어 + 날짜 + 결과값
date = datetime.datetime.now().strftime('%m%d_%H%M')   # 실행한 날짜_시간 (예: 0914_1850)
filepath = ''.join([path, 'k32_08_', date, '_', f'{result[0]:.4f}', '.keras'])   # x_test loss 를 소수 4자리로
# 예) ./_save/keras31/k32_08_0914_1850_0.1234.keras -> 이름만 보고 어떤 결과를 낸 모델인지 알 수 있다
model.save(filepath)
print('모델 저장 :', filepath)

# ===== 실행 결과 (2026-09-14, keras31 실행 직후 keras32 실행) =====
# loss : 0.08734525740146637
# acc : 0.981
# r2 : 0.9409171359033212
# mse : 0.012855110919946235
# RMSE : 0.11338038154789494
