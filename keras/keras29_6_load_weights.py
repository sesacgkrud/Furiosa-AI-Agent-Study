# keras29_6_load_weights.py
# load_weights() : 가중치만 불러온다 -> 저장할 때와 똑같은 모델 구조를 먼저 만들어 둬야 한다
# 구조가 다르면 불러오는 순간 에러가 난다
# compile 은 필요하다 (evaluate 가 loss 를 계산해야 하므로). 다시 fit 할 필요는 없다

# keras29_5_save_weights.py 베이스
...
# model.save_weights(path + 'keras29_5_save1.weights.h5') # 훈련 전, 현재까지는 훈련되지 않은 weights 저장 -> 초기 가중치

# model = load_model(path + 'keras29_1_save_model.keras')
# model.load_weights(path + 'keras29_5_save1.weights.h5') # 에러 발생
...
# model.save_weights(path + 'keras29_5_save2.weights.h5') # 훈련 후, 훈련된 weights 저장
