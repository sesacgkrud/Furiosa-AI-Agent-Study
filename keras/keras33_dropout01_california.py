# keras33_dropout01_california.py
# 구조 : keras30_ModelCheckPoint1.py 기준 + Dropout 적용 (함수형 모델)
...
path = './_save/keras33/'   # 모델 저장 폴더 (keras30 폴더에 저장하면 keras30_mcp1.keras 를 덮어쓰므로 keras33 으로 분리)
...
#2. 모델 구성 (함수형 모델 : 층을 변수로 만들고 뒤에 (이전 층) 을 붙여서 연결한다)
input1 = Input(shape=(8,))                      # 입력층 : 컬럼 8개
dense1 = Dense(100, activation='relu')(input1)  # input1 뒤에 연결
drop1 = Dropout(0.2)(dense1)                    # dense1 출력의 20% 를 훈련 때마다 랜덤으로 끈다
...
mcp = ModelCheckpoint(
    ...
    filepath=path + 'keras33_mcp1.keras',
    verbose=1,
)
...
# ===== 이전 기록 =====
# Dropout 적용 전 (keras30_ModelCheckPoint1, MCP)  loss : 2.1273159980773926
# Dropout 적용 후 (이전 실행, r2/mse/rmse 추가 전)  loss : 1.986042857170105

# ===== 실행 결과 (2026-09-14, Dropout + MCP + r2/mse/rmse 추가 후) =====
# loss : 0.2530974745750427
# r2 : 0.8080744359790722
# mse : 0.25309746873684347
# RMSE : 0.503087933404135
# (Epoch 216: early stopping / Restoring model weights from the end of the best epoch: 186.)
