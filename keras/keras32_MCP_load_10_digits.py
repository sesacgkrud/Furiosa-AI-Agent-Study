# keras32_MCP_load_10_digits.py
# 구조 : keras30_ModelCheckPoint2_load.py 기준
# 짝 파일 : keras31_MCP_save_10_digits.py -> 반드시 그 파일을 먼저 실행해서 keras31_mcp10.keras 를 만들어야 한다.
...
model = load_model(path + 'keras31_mcp10.keras')
...
# 불러온 모델 = keras31 에서 val_loss 가 가장 좋았던 epoch 의 모델이고, fit 을 안 했으니 가중치도 keras31_mcp10.keras 와 같다.
# 파일명 만들기는 keras30_ModelCheckPoint3 방식 : 폴더 + 접두어 + 날짜 + 결과값
date = datetime.datetime.now().strftime('%m%d_%H%M')   # 실행한 날짜_시간 (예: 0914_1850)
filepath = ''.join([path, 'k32_10_', date, '_', f'{result[0]:.4f}', '.keras'])   # x_test loss 를 소수 4자리로
# 예) ./_save/keras31/k32_10_0914_1850_0.1234.keras -> 이름만 보고 어떤 결과를 낸 모델인지 알 수 있다
model.save(filepath)
print('모델 저장 :', filepath)

# ===== 실행 결과 (2026-09-14, keras31 실행 직후 keras32 실행) =====
# loss : 0.2588503658771515
# acc : 0.951
# r2 : 0.9155093438350927
# mse : 0.007604828087568999
# RMSE : 0.08720566545568584
# accuracy_score : 0.9513212795549374
