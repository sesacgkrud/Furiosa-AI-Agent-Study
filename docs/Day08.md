...
> 클래스가 3개 이상인 **다중 분류** 로 확장하고, 정답 y 를 One-Hot 으로 바꾸는 3가지 방법을 비교했다.
...
| 주제 | 핵심 한 줄 |
|---|---|
| 다중 분류 3종 세트 | 출력 노드 = 클래스 개수 + `softmax` + `categorical_crossentropy` |
| softmax | 클래스별 확률을 출력하고 전부 더하면 1 |
| One-Hot 이 필요한 이유 | 0/1/2 를 그대로 쓰면 "2가 0보다 크다"는 관계를 학습해버린다 |
| One-Hot 3가지 | `to_categorical` / `pd.get_dummies` / `OneHotEncoder`(2차원 필수) |
| to_categorical 함정 | 0번부터 만들어서 y가 1~7이면 빈 0번 컬럼이 생긴다 |
| np.argmax(axis=1) | 확률 -> 클래스 번호. y_test 와 y_predict 둘 다 변환해야 한다 |
| batch_size | 데이터가 크면 크게(32~), 작으면 작게(4) |
...
### 다중 분류 3종 세트
1. 출력층 `activation='softmax'` + **출력 노드 수 = 클래스 개수**
   - `[2.3, 0.8, -1.2]` -> softmax -> `[0.80, 0.18, 0.02]` (합이 1인 확률)
2. `loss='categorical_crossentropy'` - One-Hot 정답과 softmax 예측을 비교
3. 정답 y를 **One-Hot Encoding** 으로 변환

### One-Hot Encoding - 왜, 어디서
- `[0, 1, 2]` -> `[[1,0,0], [0,1,0], [0,0,1]]`
- 숫자를 그대로 쓰면 모델이 **"2가 0보다 크다"는 크기·순서 관계**가 있다고 오해한다
- 모델이 아니라 **정답 데이터를 바꾸는 전처리**이므로 `#1. 데이터` 영역에서 처리한다

### One-Hot 3가지 방법 비교
| 방법 | 코드 | 특징 |
|---|---|---|
| TensorFlow | `to_categorical(y)` | 가장 간단, **0번 클래스부터 자동 생성** |
| pandas | `pd.get_dummies(y, dtype=int)` | DataFrame으로 다룰 때 편하다 |
| sklearn | `OneHotEncoder(sparse_output=False)` | 입력이 **2차원**이어야 해서 `y.reshape(-1,1)` 필요 |

- `to_categorical` 의 함정 : y가 1부터 시작하면(covtype 1~7) 0번 자리가 빈 **컬럼 8개** 가 생긴다

### argmax로 되돌리기
- `np.argmax(axis=1)` - One-Hot / 확률값을 **가장 큰 값의 위치(클래스 번호)** 로 되돌린다
- `accuracy_score` 에 넣기 전 **y_test와 y_predict 둘 다** 변환해야 한다

### 대용량 데이터 다루기
- `metrics=['acc']` 때문에 `evaluate` 반환값은 `[loss, acc]` -> `result[0]`, `result[1]`
- `time.time()` 으로 소요 시간 측정 (covtype 58만 행 = 1463초, 약 24분)
- `batch_size` 조절 - 소규모(digits) 4 / 대규모(covtype) 32 이상. 대용량에서 작게 주면 훈련이 끝나지 않는다

### 실습한 데이터셋 4종
- Iris(3클래스) / Wine(3클래스) / Covtype(7클래스) / Digits(10클래스)
